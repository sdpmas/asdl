...
#will ignore sepaction altogether for now. Come back later.
# class SepAction(Action):
#     def __repr__(self):
#         return 'Mask'
...
class TransitionSystem(object):
    ...
    def get_actions(self, asdl_ast=None,text=None,grammar=None):
        ...
        if asdl_ast is not None:
            # print ('text again',text)
            # print ('production: ',asdl_ast.production,type(asdl_ast.production))
        ...
        if text is not None:
            ...
            for t in text:
                # prod=grammar.get_prod_by_ctr_name('Expr')
                # prod_expr=grammar.get_prod_by_ctr_name('Str')
                # text_actions.append(ApplyRuleAction(prod))
                # text_actions.append(ApplyRuleAction(prod_expr))
        ...
        if asdl_ast is not None:
            for field in asdl_ast.fields:
                ...
                if self.grammar.is_composite_type(field.type):
                    ...
                    #TODO: modify get_primitive_field_actions.
                    # print ('gen token reached',field)
        ...
        if text is not None and asdl_ast is not None:
            # print (text_actions, 'these are the text actions')
            # print (code_actions,"these are the code actions")
            # print (actions,'these are the actions')
            # print ('length is: ', len(actions),len(text_actions),len(code_actions))
            # assert len(code_actions)+len(text_actions)==len(actions)
            assert len([code_actions[0]])+len(text_actions)+len(code_actions[1:])==len(actions)
            # assert [code_actions[0]]+text_actions+code_actions[1:]==actions
            # assert len(actions[1:][:len(text_actions)])==len(text_actions)
