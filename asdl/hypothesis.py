...
class Hypothesis(object):
    ...
    def apply_action(self, action):
        ...
        if self.tree is None:
            ...
            # print ('action: ',action,'frontier field',self.frontier_field,self.frontier_field.type)
            if isinstance(self.frontier_field.type, ASDLCompositeType):
                # print (self.frontier_field, 'frontier_field',action,'action')
                if isinstance(action, ApplyRuleAction):
                    # print ('action: ',action,'frontier field',self.frontier_field)
                ...
                if isinstance(action, GenTokenAction):
                    ...
                    end_primitive = False
                    #TODO: this might be the solution to the type name of field.
                    if self.frontier_field.type.name == 'string':
                        # print ('how often do we get here') 
                    ...
                    #gen text might not always be string. it can be any primitive type. It might turn out to be helpful for bi-directional pointer mechanism.
                    # print ('we have reached the gentext action')
                    self.frontier_field.add_value(action.text)
                    end_primitive = True

                    if end_primitive and self.frontier_field.cardinality in ('single', 'optional'):
                        self.frontier_field.set_finish()
                        self.update_frontier_info()
                #TODO: figure out a way for maskaction.
    ...
    def update_frontier_info(self):
        def _find_frontier_node_and_field(tree_node):
            if tree_node:
                # print (tree_node.fields,tree_node.fields[0].value)
