...
class Python3TransitionSystem(TransitionSystem):
    ...
    #     #a bit tricky here. We will keep the parsed code as it is. And add the parsed 'text' ourselves later. 
    ...
    # def ast_to_surface_code_and_text(self, asdl_ast):
    #     py_code_ast,text=asdl_ast_to_python_ast(asdl_ast,self.grammar)
    #     code=astor.to_source(py_code_ast).strip()
    #     #TODO: check if the text is going to be a ast as well or not
    #     if code.endswith(':'):
    #         code+='pass'
    ...
    def get_primitive_field_actions_old(self, realized_field):
        ...
        if realized_field.value is not None:
            ...
            for tok in tokens:
                #token can either be gentoken or gentext. That depends on the type of the parent node.
                #TODO: CHECK IF THE 'TYPE' VALUE TYPE IS STRING
