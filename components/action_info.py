...
def get_action_infos1(src_query, tgt_actions, force_copy=False):
    action_infos = []
    hyp = Hypothesis()
    ...
    for t, action in enumerate(tgt_actions):
        ...
        if isinstance(action, GenTokenAction):
            try:
                ...
                if force_copy: raise ValueError('cannot copy primitive token %s from source' % action.token)
        # if isinstance(action,GenTextAction) :
        #     text_action_infos.append(action_info)
        # elif isinstance(action, MaskAction):
        #     raise ValueError('why is there mask in target?')
        else:
            code_action_infos.append(action_info)
        hyp.apply_action(action)  
        action_infos.append(action_info)
