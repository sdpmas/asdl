...
class Example(object):
    def __init__(self, tgt_actions,input_actions, idx=0, src_sent=None,  tgt_code=None, tgt_ast=None, meta=None):
...
class Batch(object):
    ...
    def frontier_prod_idx(self, e,t):
        ...
        # print ('the id is: ',id)
        # print ('the id is',id)
        # print (torch.Tensor(id),'this is the original id') 
        return id
    ...
    def init_index_tensors(self):
        ...
        self.primitive_copy_token_idx_mask = np.zeros((len(self),self.max_action_num,  max(self.src_sents_len)), dtype='float32')
            ...
            for t in range(self.max_action_num):
                ...
                if t < len(e.tgt_actions):
                    ...
                    if isinstance(action, ApplyRuleAction):
                        ...
                        app_rule_mask = 1
                    # elif isinstance(action,SepAction):
                    #     pass
                        #TODO: do sum with sepaction
                    elif isinstance(action,GenTextAction):
                        print ('not expected at all')
                        text_idx = self.vocab.source[action.text]
                        gen_text_mask=1
        ...
        # self.text_idx_matrix = Variable(T.LongTensor(self.text_idx_matrix))
        # self.gen_text_mask = Variable(T.FloatTensor(self.gen_text_mask))
