...
import sys
sys.path.insert(0, '/Users/samipdahal/asdl/asdl')
...
from components.vocab import Vocab, VocabEntry #vocab: we gonna have rougly the same structure of vocab.
...
def preprocess_conala_dataset(train_file, test_file, grammar_file, src_freq=3, code_freq=3,
    ...
    train_examples = preprocess_dataset(train_file, name='train', transition_system=transition_system,grammar=grammar)
    ...
    test_examples = preprocess_dataset(test_file, name='test', transition_system=transition_system,grammar=grammar)
    ...
    primitive_tokens = [map(lambda a: a.action.token,
                        ...
                        for e in train_examples]+[e.src_sent for e in train_examples]+[transition_system.tokenize_code(e.tgt_code, mode='decoder') for e in train_examples]
...
def preprocess_dataset(file_path, transition_system, name='train', firstk=None,grammar=None):
    try:
    ...
    for i, example_json in enumerate(dataset):
        ...
        input_actions=[]
        # print ('text',text)
        tgt_actions,text_actions,code_actions = transition_system.get_actions(asdl_ast=tgt_ast,text=text,grammar=grammar)

        # print (tgt_actions)
        # tgt_actions=transition_system.get_actions(asdl_ast=tgt_ast)
        # text_actions=[]
        # code_actions=[]
        input_actions.append(MaskAction())
        for t in text_actions:
            input_actions.append(t)
        # input_actions.append(SepAction)
        masked_actions=[MaskAction()]*(len(code_actions)-1)
        input_actions.extend(masked_actions)
        # print (len(input_actions)),
        # print (len(tgt_actions))
        # print ('input_actions: ',input_actions)
        # print ('target_actions: ',tgt_actions)
        assert len(input_actions)==len(tgt_actions)
        # print ('samip')
        ...
        tgt_action_infos,code_action_infos, text_action_infos = get_action_infos(example_dict['intent_tokens'], tgt_actions)
        # print ('samip')
        ...
        example = Example(tgt_actions=tgt_action_infos,
                          ...
                          text_actions=text_action_infos,
                          code_actions=code_action_infos,
...
#python datasets/conala/dataset1.py --topk 1000